...
//for cat 
Texture2D catRunTex;
Texture2D catSleepTex;

int catFrame = 0;
float catFrameTime = 0.0f;
float catFrameSpeed = 0.08f;
...
std::array<float, MAX_SAMPLES> cpuUsageHistory = {0};
std::array<float, MAX_SAMPLES> memUsageHistory = {0};
...
const int TEMP_SMOOTHING_SAMPLES = 5;
std::array<float, TEMP_SMOOTHING_SAMPLES> avgTempHistory = {0};
int avgTempIndex = 0;
int avgTempCount = 0;
...
float GetAvgCPUTemperature() {
    ...
    if (result.empty()) {
        cerr << "Failed to get avg temperature: sensors command returned empty output" << endl;
        return 0.0f;
    }
    ...
    while (getline(iss, line)) {
        if (line.find("coretemp-") != string::npos || line.find("k10temp-") != string::npos || 
            line.find("cpu_thermal-") != string::npos) {
            inCpuSection = true;
    ...
    avgTempIndex = (avgTempIndex + 1) % TEMP_SMOOTHING_SAMPLES;
    if (avgTempCount < TEMP_SMOOTHING_SAMPLES) avgTempCount++;

    float sum = std::accumulate(avgTempHistory.begin(), avgTempHistory.begin() + avgTempCount, 0.0f);
...
float GetDiskUsage() {
    struct statvfs stat;
    if (statvfs("/", &stat) != 0) {
        cerr << "Error: statvfs failed for root filesystem" << endl;
        return 0.0f;
    }
...
float GetMemoryUsage() {
    ...
    if (!fp) {
        cerr << "Error: Failed to open /proc/meminfo" << endl;
        return 0.0f;
    }
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        else if (strncmp(line, "MemFree:", 8) == 0) sscanf(line + 8, "%ld", &free);
    ...
    if (total == 0) {
        cerr << "Error: MemTotal is zero in /proc/meminfo" << endl;
        return 0.0f;
    }
...
float GetCPUUsage() {
    ...
    if (fscanf(fp, "cpu %ld %ld %ld %ld %ld %ld %ld %ld",
        ...
        cerr << "Error: Failed to parse /proc/stat" << endl;
        fclose(fp);
        return 0.0f;
    }
    fclose(fp);
...
void GetUsageData(float* cpuUsage, float* memUsage, float* diskUsage, float* cpuTemp, float* maxCpuTemp, float* uptimeSeconds) {
    ...
    *diskUsage = GetDiskUsage();
    ...
    string tempResult = ExecuteCommand("cat /sys/class/thermal/thermal_zone0/temp");
    float temp = 0.0f;
    if (!tempResult.empty()) {
        try {
            temp = stof(tempResult) / 1000.0f;
        } catch (...) {
            temp = 0.0f;
        }
    }
    ...
    *uptimeSeconds = uptime;
    ...
    if (*maxCpuTemp > 80.0f) {
        string msg = "High CPU Temp: " + to_string(static_cast<int>(*maxCpuTemp)) + "°C";
        SendToServer(msg);
    }
}
...
bool KillProcess(int pid, string& errorMsg) {
    ...
    if (kill(pid, 0) != 0) {
        errorMsg = "Process with PID " + to_string(pid) + " not found: " + strerror(errno);
        return false;
    }

    if (kill(pid, SIGTERM) != 0) {
        errorMsg = "Failed to send SIGTERM to PID " + to_string(pid) + ": " + strerror(errno);
        return false;
    }
    ...
    if (kill(pid, 0) == 0) {
        if (kill(pid, SIGKILL) != 0) {
            errorMsg = "Failed to send SIGKILL to PID " + to_string(pid) + ": " + strerror(errno);
            return false;
        }
...
void ProcessServerCommands(bool& processesNeedRefresh, int& selectedProcessIndex, string& killErrorMsg, double& killMsgDisplayTime) {
    ...
    if (select(clientSock + 1, &readfds, nullptr, nullptr, &tv) > 0) {
        char buffer[1024] = {0};
        int bytes = read(clientSock, buffer, sizeof(buffer) - 1);
        if (bytes <= 0) {
            cerr << "Server disconnected: " << strerror(errno) << endl;
            close(clientSock);
            clientSock = -1;
            return;
        }
        ...
        if (command == "shutdown") {
            ...
            close(clientSock);
            clientSock = -1;
            cout << "Server requested exit." << endl;
        }
    }
}
...
void DrawProcessList(const vector<ProcessInfo>& processes, Rectangle bounds, float* scrollY, int* selectedProcessIndex, bool isDarkMode) {
    ...
    Color tint = isDarkMode ? Color{200, 200, 200, 255} : BLACK;
    ...
    BeginScissorMode(static_cast<int>(bounds.x), static_cast<int>(bounds.y), static_cast<int>(bounds.width - (contentHeight > bounds.height ? 8 : 0)), static_cast<int>(bounds.height));
...
// Modern button
bool endtaskButton(Rectangle bounds, const char* text, bool enabled, bool isDarkMode, Color baseColor = {200, 200, 200, 255}) {
    bool clicked = false;
    Vector2 mousePos = GetMousePosition();
    ...
    DrawTextEx(font, text, {bounds.x + bounds.width/2 - textSize.x/2, bounds.y + bounds.height/2 - textSize.y/2}, 20, 2, isDarkMode ? Color{200, 200, 200, 255} : (enabled ? BLACK : GRAY));
...
// Modern icon button
bool refreshbutton(Rectangle bounds, Texture2D icon, bool isDarkMode, Color baseColor = {200, 200, 200, 255}) {
    bool clicked = false;
    Vector2 mousePos = GetMousePosition();
    ...
    DrawTexturePro(icon, iconSrc, 
                   {bounds.x + bounds.width/2 - 16, bounds.y + bounds.height/2 - 16, 32, 32}, 
                   {0, 0}, 0, WHITE);
...
void DrawPieChart(Rectangle bounds, float diskUsage, bool isDarkMode) {
    ...
    string diskStats = "Disk Usage:\nUsed: " + to_string(static_cast<int>(diskUsage)) + "%\nFree: " + to_string(static_cast<int>(100.0f - diskUsage)) + "%";
    DrawTextEx(font, diskStats.c_str(), {bounds.x, bounds.y - 80}, 18, 2, isDarkMode ? Color{200, 200, 200, 255} : BLACK);
}

void DrawCatAtUsage(bool running, Rectangle cpuGraphBounds)
{
    Texture2D tex = running ? catRunTex : catSleepTex;

    int x = cpuGraphBounds.x + cpuGraphBounds.width - 300;
    int y = cpuGraphBounds.y - 90;

    Vector2 size = {110, 110}; // slightly bigger than icon but not huge

    if (running)
    {
        catFrameTime += GetFrameTime();
        if (catFrameTime >= catFrameSpeed)
        {
            catFrame = (catFrame + 1) % 8;
            catFrameTime = 0;
        }

        int frameWidth = tex.width / 8;

        Rectangle src = {
            (float)(catFrame * frameWidth),
            0,
            (float)frameWidth,
            (float)tex.height
        };
...
// Draw usage graphs
void DrawUsageGraphs(Rectangle bounds, Font font, float diskUsage, bool isDarkMode, float currentTemp, float maxCpuTemp, float uptimeSeconds) {
    ...
    Rectangle cpuGraphBounds = {bounds.x + 30, bounds.y + 120, (bounds.width - 100) / 2, 150};
   
    DrawRectangleRounded(cpuGraphBounds, 0.2f, 8, isDarkMode ? Color{0, 0, 0, 255} : Color{230, 230, 230, 255});
    DrawCatAtUsage(catRunning, cpuGraphBounds); 
    for (int i = 1; i <= 4; i++) {
        float y = cpuGraphBounds.y + (cpuGraphBounds.height * i / 5);
        DrawLine(static_cast<int>(cpuGraphBounds.x), static_cast<int>(y), 
                 static_cast<int>(cpuGraphBounds.x + cpuGraphBounds.width), static_cast<int>(y), isDarkMode ? Color{100, 100, 100, 100} : Color{200, 200, 200, 100});
    }
    ...
    for (int i = 1; i <= 4; i++) {
        ...
        DrawLine(static_cast<int>(memGraphBounds.x), static_cast<int>(y), 
                 static_cast<int>(memGraphBounds.x + memGraphBounds.width), static_cast<int>(y), isDarkMode ? Color{100, 100, 100, 100} : Color{200, 200, 200, 100});
    }
    ...
    string memStats = "Memory Usage:\nCurrent: " + to_string(static_cast<int>(currentMem)) + "%\nMin: " + to_string(static_cast<int>(minMemUsage)) + "%\nMax: " + to_string(static_cast<int>(maxMemUsage)) + "%";
    ...
    Color tempColor = (currentTemp > 85.0f) ? RED : (isDarkMode ? Color{200, 200, 200, 255} : BLACK);
    string tempText = "CPU Temperature: " + to_string(static_cast<int>(currentTemp)) + "°C";
    DrawTextEx(font, tempText.c_str(), {bounds.x + 30, memGraphBounds.y + memGraphBounds.height + 30}, 18, 2, tempColor);

    Color avgTempColor = (maxCpuTemp > 85.0f) ? RED : (isDarkMode ? Color{200, 200, 200, 255} : BLACK);
    string avgTempText = "Avg CPU Temperature: " + to_string(static_cast<int>(maxCpuTemp)) + "°C";
...
void DrawContentArea(MenuOption selectedMenu, vector<string>& specsLines, vector<ProcessInfo>& processes, 
    ...
    // process page 
        ...
        if (IsKeyPressed(KEY_TAB) && selectedProcessIndex == -1 && !processes.empty()) {
        ...
        if (processSelected) {
            ...
            if (pid <= 100) {
                DrawTextEx(boldfont, "Warning: Low PID may be a system process!", {endTaskButtonRect.x, endTaskButtonRect.y + endTaskButtonRect.height + 30}, 16, 2, RED);
            }
            
            if (endtaskButton(endTaskButtonRect, "End Task", true, isDarkMode)) {
            ...
            endtaskButton(endTaskButtonRect, "End Task", false, isDarkMode);
        }
        ...
        DrawUsageGraphs(contentArea, font, diskUsage, isDarkMode, currentTemp, maxCpuTemp, uptimeSeconds);
    }
}
...
// Update processes
void UpdateProcesses(MenuOption selectedMenu, vector<ProcessInfo>& processes, bool& processesNeedRefresh, 
                     double& lastProcessRefreshTime, string& killErrorMsg, double& killMsgDisplayTime) {
    double currentTime = GetTime();
...
// Update usage data
void UpdateUsageData(MenuOption selectedMenu, int& usageUpdateCounter, float& diskUsage, float& currentTemp, float& maxCpuTemp, float& uptimeSeconds) {
    if (selectedMenu == USAGE) {
        usageUpdateCounter++;
        
        if (usageUpdateCounter >= 60) {
            float cpuUsage, memUsage, cpuTemp, maxTemp, uptime;
            GetUsageData(&cpuUsage, &memUsage, &diskUsage, &cpuTemp, &maxTemp, &uptime);
            ...
            float latestCPU = cpuUsageHistory[(usageSampleIndex - 1 + MAX_SAMPLES) % MAX_SAMPLES];
            catRunning = (latestCPU > 80.0f);
...
void InitializeResources(Texture2D& specsIcon, Texture2D& processIcon, Texture2D& usageIcon, Texture2D& exitIcon, Texture2D& refreshIcon, Texture2D& darkModeIcon) {
    ...
    //for cat
    catRunTex = LoadTexture("run.png");
    catSleepTex = LoadTexture("sleep.png");

    SetTextureFilter(catRunTex, TEXTURE_FILTER_BILINEAR);
    SetTextureFilter(catSleepTex, TEXTURE_FILTER_BILINEAR);
}
...
void CleanupResources(Texture2D specsIcon, Texture2D processIcon, Texture2D usageIcon, Texture2D exitIcon, Texture2D refreshIcon, Texture2D darkModeIcon) {
    ...
    UnloadTexture(catRunTex);
    UnloadTexture(catSleepTex);
...
int main() {
    ...
    while (!WindowShouldClose()) {
        ...
        UpdateUsageData(selectedMenu, usageUpdateCounter, diskUsage, currentTemp, maxCpuTemp, uptimeSeconds);
